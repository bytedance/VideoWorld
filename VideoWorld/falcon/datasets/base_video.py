...
class BaseVideoDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.object_min_length = object_min_length
        ...
        self.data_infors = self.load_data_list()
    ...
    def __getitem__(self, idx):
        ...
        item['data_prefix'] = self.data_prefix
    ...
    def load_data_list(self) -> Tuple[List[dict], List]:
        ...
        video_map = {vid['id']:vid for vid in ann_file['videos']}
        index = 0
        # import pdb;pdb.set_trace()
        for idx, ann in enumerate(annotations):
            print(idx)
            tmp_ann = copy.deepcopy(ann)
            valid_length = sum([box is not None for box in ann['bboxes']])
            valid_frame_idx = [i for i, box in enumerate(ann['bboxes']) if box is not None ]
            if valid_length < self.object_min_length:
                continue
            ...
            if self.test_mode:
                # import pdb;pdb.set_trace()
                for start_idx in range(0, valid_length, self.test_frame_num):
                    if start_idx + self.test_frame_num > valid_length:
                        continue
                    valid_frame_names = [video['file_names'][i] for i in valid_frame_idx[start_idx:start_idx+self.test_frame_num]]
                    tmp_ann['valid_frame_names'] = valid_frame_names
                    tmp_ann['bboxes'] = [box for i, box in enumerate(ann['bboxes']) if i in valid_frame_idx[start_idx:start_idx+self.test_frame_num]]
                    tmp_ann['segmentations'] = [segm for i, segm in enumerate(ann['segmentations']) if i in valid_frame_idx[start_idx:start_idx+self.test_frame_num]]
                    tmp_ann['height'] = ann['height']
                    tmp_ann['width'] = ann['width']
                    tmp_ann['index'] = index
                    index += 1
                    out_list.append(tmp_ann)
                    assert len(valid_frame_names) == self.test_frame_num
                    tmp_ann = copy.deepcopy(ann)
            else:
                valid_frame_names = [video['file_names'][i] for i in valid_frame_idx]
                tmp_ann['valid_frame_names'] = valid_frame_names
                tmp_ann['bboxes'] = [box for box in ann['bboxes'] if box is not None]
                tmp_ann['segmentations'] = [segm for box, segm in zip(ann['bboxes'], ann['segmentations'])  if box is not None]
                tmp_ann['height'] = ann['height']
                tmp_ann['width'] = ann['width']
                tmp_ann['index'] = index
                index += 1
