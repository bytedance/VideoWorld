...
TERO_INCEPTION_URL = './work_dirs/init/fvd/i3d_torchscript.pt'  # noqa
...
def prepare_i3d_feat(dataloader: DataLoader,
    ...
    if use_web:
        ...
        # inception_dataloader = wds.WebLoader(
        ...
        #     # **dataloader_cfg
        #     )
        # inception_dataloader = inception_dataloader.with_epoch(len(dataset)).with_length(len(dataset))
    else:
        inception_dataloader = DataLoader(
            dataset,
    ...
    # import pdb;pdb.set_trace()
    num = 0
    for data in tqdm(inception_dataloader):
        # if is_main_process():
        #     if is_slurm:
        #         pbar.update(1)
        #     else:
        ...
        # import pdb;pdb.set_trace()
        data_samples = data_preprocessor(data, False)
        #data_preprocessor(data, False)['mp4'][0]
        # real_key = 'gt_img' if metric.real_key is None else metric.real_key
        real_key = 'mp4'
        if data_samples.get('data_samples') is not None:
            img = [data_sample.gt_img for data_sample in data_samples.get('data_samples')]
            img = torch.stack(img) #b, t, c, h, w
            # img = img[0]
            # for idx, i in enumerate(img.flatten(0, 1).permute(0, 2, 3, 1)):
                # cv2.imwrite('/opt/tiger/visualize/image_{}_{}.jpg'.format(num, idx), i.cpu().numpy()[:, :, ::-1])
            
            img = img.permute(0, 2, 1, 3, 4)
        else:
            img = data_samples.get(real_key)
            # import pdb;pdb.set_trace()
            # img = img[0]
            # for idx, i in enumerate(img.flatten(0, 1)):
            #     cv2.imwrite('/opt/tiger/visualize/image_{}_{}.jpg'.format(num, idx), i.cpu().numpy()[:, :, ::-1])
            # we need to formulate to [b,c,t,w,h]
            img = img.permute(0, 4, 1, 2, 3).float() #[b,c,t,h,w]
        num += 1
        # img = img.permute(0,2,1,3,4)
    ...
    if is_main_process():
        ...
        print(len(torch.cat(real_feat, dim=0)), num_items)
        if capture_mean_cov:
